...
**スライドショーは「全画面表示」ではなく、必ず「ウィンドウ表示」で開始してください。全画面のスライドショーではコメントは流れません。**

`スライドショー` ボタン右の `▼` から **「ウィンドウ表示で開始」** を選びます。

ウィンドウ表示でもスライドはほぼ画面いっぱいに表示されるので、発表用途では十分使えます。ブラウザの UI も消したい場合は、**ウィンドウ表示のスライドショーにしたうえで Chrome 自体を全画面**（macOS は `⌃⌘F`、Windows は `F11`）にしてください。この方法ならスライドだけの表示になり、かつコメントも流れます。
